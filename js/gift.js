...
function createGiftCard(item) {
  ...
  card.innerHTML = `
    ...
    <div class="gift-item-body">
      ...
      <p class="gift-item-desc">${item.desc}</p>
      <div class="gift-item-costs">
        <span class="gift-badge">💎 ${item.points} 點</span>
        <span class="gift-badge">🎟 ${item.tickets} 券</span>
      </div>
      <div class="gift-item-actions">
        <button class="gift-redeem-btn" data-gift-id="${item.id}" ${canRedeem ? '' : 'disabled'}>
          ${canRedeem ? '立即兌換' : '資源不足'}
        </button>
      </div>
    </div>
...
function renderGiftGrid() {
  ...
  giftGridEl.querySelectorAll('.gift-redeem-btn').forEach((button) => {
