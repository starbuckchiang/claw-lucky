// Auth-07: PayPal Orders v2 one-time checkout Edge Function.
// Independent from subscription-checkout (placeholder Auth gate untouched).
// NOT deployed by this task. Sandbox secrets only when later approved.
...
import {
  ...
  createPaypalClient,
} from "../_shared/paypal-checkout-handler.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const result = await handlePaypalCheckoutRequest({
      body,
      user,
      correlationId,
      deps: {
        paymentOrdersRepository: createPaymentOrdersRepositoryFromSupabase(serviceClient),
