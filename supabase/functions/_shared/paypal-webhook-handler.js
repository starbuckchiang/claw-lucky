...
/**
 * Auth-07 / Auth-07A.1 / Auth-07C.4 paypal-webhook shared handler (Node twin).
 * Verify signature FIRST; never mutate payment data on verification failure.
 * Orders v2 path unchanged. Subscription events route to subscription processor.
 */
...
function readPaypalHeader(headers, name) {
  if (!headers) return "";
...
function extractResourceFields(event) {
  const resource = event?.resource || {};
  const eventType = String(event?.event_type || "");

  if (eventType.startsWith("PAYMENT.CAPTURE.")) {
    const money = extractCaptureMoney(resource);
    // Auth-07A.1: order id ONLY from related_ids (no other fallbacks).
...
function isSubscriptionEventType(eventType) {
  return SUBSCRIPTION_EVENT_TYPES.has(String(eventType || "").trim());
}

function targetStatusForSubscriptionEvent(eventType, resource) {
  ...
  if (type === "BILLING.SUBSCRIPTION.UPDATED") {
    ...
    if (["APPROVAL_PENDING", "APPROVED", "ACTIVE", "SUSPENDED", "CANCELLED", "EXPIRED"].includes(status)) {
      return status;
    }
    return null;
  }
...
function extractSaleFields(resource) {
  const amountTotal = resource?.amount?.total
...
function extractSubscriptionResourceFields(event) {
  const resource = event?.resource || {};
  const eventType = String(event?.event_type || "");
...
function sanitizeSubscriptionPayload({
  eventType,
  ...
  const payload = {
    ...
    paypal_sale_id: paypalSaleId || null,
    amount: amount != null ? Number(amount) : null,
    currency: currency || null,
    payment_time: paymentTime || null,
    next_billing_time: nextBillingTime || null,
    target_status: targetStatus || null,
    is_full_refund: isFullRefund === true,
    plan_id: planId || null
  };
  // Never include payer / subscriber / email / shipping.
  return payload;
}
...
async function persistVerifiedReceived(repo, {
  paypalEventId,
  eventType,
  paypalSubscriptionId,
  paypalSaleId,
...
async function markWebhookFailure(repo, {
  paypalEventId,
  errorCode,
...
async function handlePaypalWebhookRequest({
  rawBody,
  headers,
  deps = {}
}) {
  ...
  if (!authAlgo || !certUrl || !transmissionId || !transmissionSig || !transmissionTime) {
    return {
      statusCode: 401,
      body: { ok: false, error: { code: "WEBHOOK_SIGNATURE_MISSING", message: "Missing PayPal signature headers." } }
    };
  }
  ...
  try {
    ...
    return {
      statusCode: 401,
      body: { ok: false, error: { code: "WEBHOOK_VERIFY_FAILED", message: "Webhook verification failed." } }
    };
  }
  ...
  if (String(verification.verification_status) !== "SUCCESS") {
    return {
      statusCode: 401,
      body: { ok: false, error: { code: "WEBHOOK_VERIFY_FAILED", message: "Invalid webhook signature." } }
    };
  }

  let event = verification.webhookEvent;
  ...
  // Auth-07C.4: subscription events after successful signature verification.
  ...
  try {
    ...
    // Unfixable validation failures: HTTP 200 so PayPal does not retry forever.
    // Event is stored as processing_status=failed; payment_orders not marked paid.
    if (outcome === "rejected") {
      return {
        statusCode: 200,
        body: {
...
function createWebhookRepositoryFromSupabase(serviceClient) {
  return {
...
module.exports = {
  ...
  extractSubscriptionResourceFields,
